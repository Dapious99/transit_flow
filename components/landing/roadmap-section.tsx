...
const statusConfig = {
  completed: { icon: CheckCircle2, color: 'text-green-500', bg: 'bg-green-500/10', border: 'border-green-500/30', label: 'Completed' },
  in_progress: { icon: Clock, color: 'text-primary', bg: 'bg-primary/10', border: 'border-primary/40', label: 'In Progress' },
  upcoming: { icon: Circle, color: 'text-muted-foreground', bg: 'bg-secondary', border: 'border-border', label: 'Upcoming' },
}
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground leading-relaxed">
            A transparent, phased plan — from MVP and beta testing today to GPU-accelerated
            Pan-African scale by 2026.
          </p>
        </motion.div>
          ...
          {phases.map((phase, index) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mb-4">{phase.quarter}</p>

                <ul className="space-y-2">
                  {phase.milestones.map((milestone) => (
                    <li key={milestone} className="flex items-start gap-2 text-xs text-muted-foreground">
